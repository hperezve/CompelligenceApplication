 ...
 * Dashboard Service
 */
app.service("PoReviewFinalizeService", function(PuridiomData, PurdiomConfig, $http, $q, $log) {

	// Set Request Parameters for Dashboard Service
	this.setPuridiomHeader = function() {
		PuridiomData.request.requestHeader.module = 'order';
		PuridiomData.request.requestHeader.service = 'open';
		PuridiomData.request.requestHeader.pageId = 'POPO_REVIEW';
	...
	this.setServiceParameters = function(routeParams){
		...
		PuridiomData.request.requestBody = {
			PoObject : {
				...
				object : {
					header: {
						icPoHeader: routeParams.icPoHeader
					}
				}
			}
		};
