 ...
 * Dashboard Service
 */
app.service("TwoColLineService", function(PuridiomData, PurdiomConfig, $http, $q, $log) {
	...
	this.modifyCommentData = function(dataObject, routeParams, service) {
		...
		PuridiomData.request.requestHeader.service = 'putCacheLineComments';
		PuridiomData.request.requestHeader.pageId = routeParams.pageId;
		PuridiomData.request.requestHeader.moduleRequest = 'request';
		PuridiomData.request.requestHeader.processKey = '';
		PuridiomData.request.requestHeader.browseKey = '';
		...
		PuridiomData.request.requestBody = {
			RequisitionObject : {
				module : 'requisition',
				service : 'putCacheLineComments',
				object : {
					header : {
						icReqHeader : routeParams.icReqHeader
					},
					lines : [dataObject.RequisitionLineObject],
				}
			},
			BrowseObject : {
				module : 'browse',
	...
	this.saveCache = function(dataObject) {
		...
		requestObject.requestHeader = PuridiomData.request.requestHeader;
		requestObject.requestBody = dataObject.RequisitionObject;
		$log.debug("dataObject: ");
		$log.debug(dataObject);
		$log.debug("requestObject: " + requestObject);
		$log.debug(requestObject);
	...
	this.putCacheData = function(dataObject, routeParams) {
		...
		$log.debug("Set header in PuridiomData.request:");
		PuridiomData.request.requestHeader.module = 'requisition';
		PuridiomData.request.requestHeader.pageId = 'REQ_LINE_PAGE';
		PuridiomData.request.requestHeader.service = 'putCacheLine';
		PuridiomData.request.requestHeader.moduleRequest = 'request';
		PuridiomData.request.requestHeader.processKey = '';
		PuridiomData.request.requestHeader.browseKey = '';
		$log.debug(PuridiomData.request);
		...
		PuridiomData.request.requestBody = {
			RequisitionLineObject : {
				module : 'requisition',
				service : 'putCacheLine',
				object : {
					header: {
						icReqHeader: routeParams.icReqHeader
					},
					lines: [{line: dataObject.RequisitionLineObject.line}]
				}
			},
			RequisitionObject : {
				module : 'requisition',
	...
	// Save document in repository with cache update,
	// the document is update in repository and cache
	this.uploadAttachments = function(dataObject,routeParams) {
		...
		PuridiomData.request.requestBody = {
			RequisitionObject : {
				...
				object : {
					...
					lines: [
					        	{attachments: dataObject.RequisitionLineObject.attachments,
					        	line: dataObject.RequisitionLineObject.line}
					       ]
				}
			},
		};
